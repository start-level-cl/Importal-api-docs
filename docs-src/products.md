...
> [!IMPORTANT]
> **Lista blanca de datos deliberada.** `PublicProductDto` expone únicamente `id`, `nombre` (= `product.marca`, no existe un campo "nombre" separado en la entidad `Product`), `fotos` (URLs de CloudFront ya resueltas, sin firma), `precioClp`, `proveedorNombre`, `tipo` (`"aereo" | "maritimo" | "both"`), `tallas` y `disponible`. **No** expone `proveedorCodigo`/`external_id` del vendor, `vendor_id`, costos internos, comisiones ni el `status` del producto — a propósito, por seguridad y privacidad de datos de negocio.
>
